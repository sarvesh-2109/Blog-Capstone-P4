...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        ...
        new_comment = Comment(
            ...
            parent_post=requested_post,  # This line correctly assigns the post_id
        )
        db.session.add(new_comment)
        db.session.commit()
        return redirect(url_for("show_post", post_id=post_id))  # Redirect to avoid resubmission on refresh
    ...
    if request.method == "POST":
        with smtplib.SMTP("smtp.gmail.com") as connection:
            ...
            connection.sendmail(
                from_addr=MY_EMAIL,
                to_addrs=MY_EMAIL,
                msg=f"Subject:New Message from {request.form['name']}\n\n"
